...
do
  ...
  cp ./01_RawCDSFiles/$speciesCode"_cds.fasta" ./03_OrthoFinder/fasta/
  # Append a species code to each gene name:
  export samplePrefix=$speciesCode"_"
  sed -i .backup "s/>/>$samplePrefix/g" "./03_OrthoFinder/fasta/"$speciesCode"_cds.fasta"
  ...
  sed -i .backup '/^>/! s/\./-/g' "./03_OrthoFinder/fasta/"$speciesCode"_cds.fasta"
  rm ./03_OrthoFinder/fasta/*.backup
done

# Download the Bombyx mori proteins as an outgroup:
wget https://ftp.ncbi.nlm.nih.gov/genomes/all/GCF/030/269/925/GCF_030269925.1_ASM3026992v2/GCF_030269925.1_ASM3026992v2_cds_from_genomic.fna.gz -O bomo.gz
gunzip -c bomo.gz > ./03_OrthoFinder/fasta/bomo_cds.fasta
...
orthofinder -S diamond -t $1 -f ./03_OrthoFinder/fasta/ -d
